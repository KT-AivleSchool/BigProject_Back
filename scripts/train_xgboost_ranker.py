...
def train_and_save_model():
    ...
    if db_result is not None:
        X_db, y_db = db_result
        print(
            f"[DB] 실제 사용자 피드백 데이터({X_db.shape[0]}건)를 성공적으로 불러왔습니다!"
        )
        print(
            "[Hybrid] 더미 데이터와 실제 데이터를 병합(Concatenate)하여 학습을 진행합니다."
        )
        ...
        print(
            "[Dummy] DB에 학습할 진짜 데이터가 아직 없습니다. [더미 데이터]만으로 초기(콜드스타트) 학습을 진행합니다."
        )
        X, y = X_dummy, y_dummy
