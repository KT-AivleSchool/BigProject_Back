...
def generate_dummy_data(extract_features_func):
    ...
    raw_data = [
        ...
        ("흡연부스 설치 기준", "흡연부스 설치 시 주변 건물과 50m 이상 거리를 두어야 한다.", 0.85, 1),
        ("전기차 충전소 규제", "전기차 충전소는 주차장 면적의 5% 이상을 할당해야 하며 100제곱미터 이상의 공간이 필요하다.", 0.90, 1),
        ("청년주택 혜택 용적률", "청년주택 건설 시 용적률 500% 혜택을 제공하며 역세권 250m 이내여야 합니다.", 0.88, 1),
        
        # 나쁜 문서 (정답=0): 길이는 길지만 쿼리 키워드 부족, 수치 조건 없음
        ("흡연부스 설치 기준", "최근 도심 내 공원 환경 조성을 위해 다양한 나무를 심고 시민들에게 휴식 공간을 제공하자는 논의가 활발히 진행 중입니다.", 0.35, 0),
        ("전기차 충전소 규제", "친환경 자동차 보급이 늘어나면서 시민들의 대기 오염에 대한 관심이 높아지고 있습니다. 이에 따라 지자체는 캠페인을 벌입니다.", 0.40, 0),
        
        # 애매한 문서 (정답=0): 키워드는 좀 있지만 수치 조건이나 구체성 결여, 혹은 너무 짧음
        ("청년주택 혜택 용적률", "청년주택 관련 조례입니다.", 0.50, 0),
        ("전기차 충전소 규제", "전기차와 관련된 일반적인 안전 수칙 안내문입니다.", 0.45, 0),
        
        # 🚨 함정 데이터 1 (정답=0): 벡터 점수는 매우 높지만, 막상 핵심 키워드나 수치 조건이 없는 껍데기 문서
        ("흡연부스 설치 기준", "흡연과 무관한 일반적인 시설물 설치 기준입니다. 다양한 조례에서 공통적으로 적용되는 위치 제약 조건만을 설명하고 있습니다.", 0.88, 0),
        ("건축 허가 면적", "건축에 관한 허가 기준을 다루는 안내문서로, 구체적인 기준이나 제약 사항은 추후 고시됩니다.", 0.92, 0),
    ...
    for _ in range(100):
        ...
        query = base_query + " " + random.choice(["", "방법", "관련 법안", "사례", "가이드라인"])
