...
def parse_statute(
    ...
    *,
    ...
    🔴 2026-08-11. `facility_type` 은 **기본값이 없다.** 예전엔 `"흡연부스"` 였다 —
       어느 도메인의 조례를 올려도 흡연부스로 태깅됐다. 기본값은 **아무도 안 쓰면서
       다음 사람만 기다리는 함정**이었다(원칙 2). 호출자 둘(`upload.py` ·
       `ingest_statutes.py`)은 그때도 값을 정해서 넘기고 있었다.

    🔴 2026-08-24. **`None` 을 받는다**(`str` → `Optional[str]`). 예전엔 이 태그가
       검색 필터 키였다 — 콜렉션이 `statutes_collection` 하나뿐이라 태그가 격리를
       대신했고, 한 글자만 달라도 전량 0건이었다. 지금은 콜렉션이 도메인마다
       따로라(`statutes_<도메인>`) 격리가 **구조**이고 이 값은 사람이 보는
       부가정보다. 모르면 비워 둔다 — 지어내면 없는 사실을 적는 것이다(원칙 4).
    """
