...
class StakeholderGraphState(TypedDict, total=False):
    ...
    interest_profiles: List[Dict[str, Any]]      # 생성된 이해관계 프로필 목록
    diversity_issues: List[Dict[str, Any]]       # 페르소나 차별성 검사 결과 이슈 목록
    ...
    persona_contexts: Dict[str, Dict[str, Any]]  # 페르소나별 선택된 Context 매핑 (persona_id -> CandidateContext)
    ...
    factcheck_results: List[Dict[str, Any]]      # 팩트체크 판정 결과 리스트
