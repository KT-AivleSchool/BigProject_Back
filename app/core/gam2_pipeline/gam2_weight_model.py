    ...
    for r in reviewed.get("results", []):
        ...
        for role in r.get("roles") or []:
            ...
            if rt == "positive_factor":
                pos[did] = {
                    "seed_weight": float(role.get("weight", 0.5)),
                    "direction": "benefit",
                    "rationale": role.get("rationale", ""),
                }
            elif rt == "negative_factor":
                pos[did] = {
                    "seed_weight": abs(float(role.get("weight", -0.5))),
                    "direction": "cost",
                    "rationale": role.get("rationale", ""),
                }
...
def build_matrix(
    ...
    for i in indicators:
        ...
        if i["kind"] == "admin":
            ...
            vals = (
                cand["_cid"].map(cand_admcd).map(lambda cd: _admin_code_match(cd, amap))
            )
        ...
        if verbose:
            hit = int((mat[iid] > 0).sum())
            print(
                f"  [{iid}] {i['kind']:<11} R={R:>4.0f}m  hit={hit}/{len(cand)} "
