...
from app.config import ADM_DONG_SHP, CSV_ENCODINGS, STEP2_OUTPUT_DIR
...
def _load_raw(profile: dict, data_dir: str):
    ...
    if not fname:
        raise ValueError(f"profile 에 filename 없음: dataset_id={profile.get('dataset_id')}")
...
def _save_dataset(df, out_base: str, csv_preview: bool) -> tuple:
    ...
    if gpd is not None and isinstance(df, gpd.GeoDataFrame) and df.geometry.notna().any():
        p = out_base + ".gpkg"
...
def _autoresolve_whitelist(df, key_col: str, mode: str, key_index: dict,
                           min_keys: int = 3, min_cover: float = 0.5) -> tuple:
    ...
    for m in modes:
        ...
        for did, cols in key_index.items():
            for col, raw_vals in cols.items():
                ...
                if hit:
                    tried.append({"dataset_id": did, "column": col, "normalize": m,
                                  "matched": hit, "cover": round(cover, 3)})
                if cover >= min_cover and hit >= min_keys and cover > best_score:
                    best, best_score, best_mode = (did, col, sorted(vals)), cover, m
    if not best:
        return None, {"reason": "값이 충분히 겹치는 컬럼을 다른 데이터셋에서 찾지 못함",
                      "후보": sorted(tried, key=lambda x: -x["cover"])[:5]}
    did, col, vals = best
    return vals, {"from_dataset": did, "from_column": col, "key_col": key_col,
                  "normalize": best_mode, "normalize_changed": best_mode != mode,
                  "matched_keys": len(vals), "cover": round(best_score, 3)}
...
def _flag_to_dict(f) -> dict:
    ...
    d = {"type": f.type, "severity": f.severity, "row_id": f.row_id,
...
def _pick_result_path() -> str:
    ...
    for name in ("audit_result_reviewed.json",
...
def clean_domain(domain_dir: str, csv_preview: bool = False,
                 prune: bool = True) -> str:
    ...
    for r in _order_datasets(results):
        ...
        try:
            ...
            emitted_here = {(o.get("params") or {}).get("name")
                            for o in ops if o.get("op_id") == "emit_whitelist"}
            circular = {(o.get("params") or {}).get("whitelist")
                        for o in ops if o.get("op_id") == "filter_by_join_key"} & emitted_here
            circular.discard(None)
            if circular:
                ops = [o for o in ops
                       if not (o.get("op_id") == "filter_by_join_key"
                               and (o.get("params") or {}).get("whitelist") in circular)
                       and not (o.get("op_id") == "emit_whitelist"
            ...
            for op in ops:
                ...
                if keys:
                    ...
                    prm["normalize"] = why.get("normalize", prm.get("normalize", "none"))
                    why["whitelist"] = nm
                    wl_resolved.append(why)
                else:
                    why["whitelist"] = nm
                    wl_resolved.append(why)
            ctx = cat.OpContext(facility=facility, region=region,
                                domain=domain_dir, adm_shp_path=ADM_DONG_SHP,
                                whitelists=wl)
            clean, flags, logs = cat.execute(df, {"cleaning_ops": ops}, ctx)
            unknown_ops = [f.raw_text for f in flags if f.type == "unknown_op"]
            skipped = unknown_ops + [f.raw_text for f in flags
                                     if f.type == "missing_whitelist"]
            skipped += [w["whitelist"] for w in wl_resolved if "reason" in w]
            skipped += [f.raw_text for f in flags
                        if f.type in ("filter_no_match", "admin_name_low_match",
                                      "admin_name_map_missing")]
            ...
            key_index.update(_index_keys(did, clean))   # 뒤 데이터셋의 조인 짝 후보로 등록
            cleaned[did] = clean
            for w in wl_resolved:                       # 생산자 정리 요청 적재
                if w.get("from_dataset"):
                    prune_reqs.append({"producer": w["from_dataset"],
                                       "producer_col": w["from_column"],
                                       "consumer": did,
                                       "consumer_col": w["key_col"],
                                       "normalize": w.get("normalize", "none")})
            out_base = os.path.join(STEP2_OUTPUT_DIR,
                                    f"{prefix + '_' if prefix else ''}clean_{did}")
            ...
            report.append({
                ...
                "op_logs": [asdict(l) for l in logs],
                "output": out_path,
                "format": fmt,
            ...
            if status == "near_empty":
                tags.append(f"!! {drop_ratio:.1%} 소실, {len(clean)}행만 남음 — 확인 필요")
            if unknown_ops:
    ...
    if prune:
        ...
        for q in prune_reqs:
            ...
            if pdf is None or cdf is None:
                continue
            if q["producer_col"] not in pdf.columns or q["consumer_col"] not in cdf.columns:
                continue
            ...
            cleaned[pdid] = pruned
            rec = by_out.get(pdid)
            base = os.path.join(STEP2_OUTPUT_DIR,
                                f"{prefix + '_' if prefix else ''}clean_{pdid}")
            _clear_stale(base)
            out_path, fmt = _save_dataset(pruned, base, csv_preview)
            print(f"  [{pdid}] 정리: {len(pdf)}→{len(pruned)}행 "
                  f"({cdid} 에 데이터 없는 {n_drop}건 제거)")
            if rec is not None:
                rec["rows_after"] = len(pruned)
                rec["output"], rec["format"] = out_path, fmt
                rec["pruned_by"] = {"consumer": cdid, "dropped": n_drop,
    ...
    with open(out_report, "w", encoding="utf-8") as f:
        json.dump({"_schema": {"설명": "STEP3 정제 실행 리포트. dataset별 정제 결과·flag·op로그.",
                               "gis_input": "위치선정 GIS 입력 대상 여부(reference_only면 false)",
                               "whitelists_생산": wl_summary},
                   "facility": facility, "region": region,
    ...
    if cnt["empty_result"]:
        print("  ※ empty_result = 결과가 0행. GIS 입력에서 그 레이어가 통째로 빠진다. "
              "cleaning_ops 의 필터 조건을 확인하세요(리포트 op_logs 참고).")
    if cnt["no_ops"]:
        print("  ※ no_ops = 감리가 cleaning_ops 를 내지 않은 데이터셋. "
...
if __name__ == "__main__":
    ...
    if not args:
        ...
        sys.exit(1)
    try:
        clean_domain(args[0], csv_preview="--csv-preview" in flags_,
