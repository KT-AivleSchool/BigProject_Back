...
def main():
    ...
    ap.add_argument(
        "--jimok", nargs="+", default=["학"], help="확인할 지목 (예: 학 공 천)"
    )
    ...
    print("-" * 68)
    print(
        f"  union 면적 {union.area / 1e6:.3f} km²  (용산구 21.87 km² 대비 "
    ...
    for jm in args.jimok:
        ...
        if miss.empty:
            print("    ✅ 전부 기존 배제에 포함 — 별도 배제 레이어 불필요")
            continue

        # 미배제 필지가 배제구역에서 얼마나 떨어져 있나
        d = gpd.GeoSeries(miss.geometry.representative_point(), crs=WORK_CRS).distance(
            union
        )
        print(f"    ⚠ 미배제 {len(miss):,}개 — 최근접 배제까지 거리:")
        print(
            f"        중앙값 {d.median():>7.0f}m   최소 {d.min():>6.0f}m   "
            f"최대 {d.max():>6.0f}m"
        )
        ...
        for _, r in show.iterrows():
            ...
            print(
                f"        {str(r['JIBUN'])[:16]:<16} {r['_d']:>6.0f}m  "
