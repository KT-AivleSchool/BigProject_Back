...
def extract_cited_laws(ordinance_rag: str) -> list[str]:
    ...
    for l in laws:
        ...
        if not l or l in seen:
            continue
        if "조례" in l or "규칙" in l and "시행규칙" not in l:
            continue
        seen.add(l)
        out.append(l)
    return out
...
def _search_law_id_uncached(law_name: str) -> tuple[str, str] | None:
    ...
    import requests
    params = {"OC": LAW_GO_KR_OC, "target": "law", "query": law_name,
              "type": "JSON", "display": 5}
    try:
...
def find_radius_in_laws(cited_laws: list[str], facility_type: str,
                        facility: str = "", model: str | None = None) -> dict:
    ...
    for law_name in targets:
        ...
        primary = [ln for ln in lines
                   if any(k in ln for k in ft_keys) and any(d in ln for d in DIST)]
        # 시설명은 있지만 거리 표현이 없는 줄(정의·범위 조항 — 문맥용)
        context = [ln for ln in lines
                   if any(k in ln for k in ft_keys) and ln not in primary]
    ...
    if not chunks:
        return {"제안값": None, "출처": None, "근거문장": "인용 상위법에서 관련 조문 미발견",
                "source_type": "law_api", "confirmed": False}
...
def _mock_llm_radius(facility_type: str, chunks: list) -> dict:
    ...
    for c in chunks:
        ...
        if m:
            return {"제안값": int(m.group(1)), "출처": c["법령"],
                    "근거문장": c["관련조문"][:100], "source_type": "law_api", "confirmed": False}
    return {"제안값": None, "출처": None, "근거문장": "(mock) 미발견",
            "source_type": "law_api", "confirmed": False}


def _mock_law_text(law_mst: str) -> str:
    if law_mst == "001234":   # 국민건강증진법(mock)
        return ("제9조(금연을 위한 조치) 어린이집·유치원 및 초중고 학교의 경계선으로부터 "
                "30미터 이내를 금연구역으로 지정한다(2024.8.17 시행).")
    return ""


def _mock_law_id(law_name: str) -> str | None:
    return {"국민건강증진법": "001234", "교육환경 보호에 관한 법률": "005678"}.get(law_name)
