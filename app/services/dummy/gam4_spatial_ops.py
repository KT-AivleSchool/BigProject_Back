...
def neighbors_within(cand: gpd.GeoDataFrame, targets: gpd.GeoDataFrame,
    ...
    거리 감쇠 점수화의 입력. sjoin 으로 쌍을 구한 뒤 거리를 벡터 계산한다.
    ...
    for st in range(0, len(c), chunk):
        ...
        buf = gpd.GeoDataFrame({"_ci": np.arange(st, st + len(seg))},
                               geometry=seg.buffer(radius).values, crs=c.crs)
        n_nan = int(buf.geometry.isna().sum())
        if n_nan:                              # fail loudly — 조용한 누락 재발 방지
            raise RuntimeError(
                f"neighbors_within: buffer geometry 결측 {n_nan}/{len(buf)} "
                f"(chunk st={st}) — 인덱스 정렬 확인 필요")
        j = gpd.sjoin(tg, buf, how="inner", predicate="within")
        if len(j) == 0:
            continue
        ci = j["_ci"].to_numpy()
        ti = j.index.to_numpy()
