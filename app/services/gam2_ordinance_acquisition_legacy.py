...
from app.config import LAW_GO_KR_OC, FACILITY_LLM_MODEL, SEARCH_CACHE_DIR, OPENAI_API_KEY
...
def search_ordinances(region: str, facility: str) -> list[dict]:
    ...
    params = {"OC": LAW_GO_KR_OC, "target": "ordin", "query": query,
...
def _mock_search(region: str, facility: str) -> list[dict]:
    ...
    return [
        {"제목": f"서울특별시 {region} 금연구역 지정 및 간접흡연피해방지 조례",
         "id": "1052687", "지자체": region, "시행일": "20251226"},
        {"제목": "서울특별시 금연환경 조성 및 간접흡연 피해방지 조례",
         "id": "2000001", "지자체": "서울특별시", "시행일": "20250101"},
        {"제목": f"서울특별시 {region} 폐기물 관리 조례",           # 노이즈
         "id": "1658985", "지자체": region, "시행일": "20251226"},
        {"제목": f"서울특별시 {region} 보건소 수가 조례",           # 노이즈
         "id": "1285808", "지자체": region, "시행일": "20240101"},
    ]


def select_ordinances(candidates: list[dict], facility: str, region: str,
    ...
    titles = [{"id": c["id"], "제목": c["제목"], "지자체": c["지자체"]} for c in candidates]
    prompt = (
...
def fetch_ordinance_text(ordinance_id: str) -> dict:
    ...
    import requests
    params = {"OC": LAW_GO_KR_OC, "target": "ordin", "MST": ordinance_id, "type": "JSON"}
    try:
...
def build_ordinance_rag(texts: list[dict]) -> str:
    ...
    for t in texts:
        ...
        for up in t.get("상위법", []):
            parts.append(f"[상위법: {up.get('법령명','')} {up.get('조항','')}] {up.get('내용','')}")
        parts.append("")
    return "\n".join(parts)
...
def acquire_ordinances(facility: str, region: str) -> dict:
    ...
    json.dump(result, open(os.path.join(ORDINANCE_CACHE_DIR, f"{key}.json"), "w",
