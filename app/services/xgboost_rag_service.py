...
class XGBoostRAGService:
    ...
    def _load_model(self):
        if xgb and os.path.exists(self.model_path):
            try:
                ...
                logger.info(f"[{self.model_path}] 경로에서 XGBoost 모델을 성공적으로 로드했습니다.")
            except Exception as e:
                logger.error(f"XGBoost 모델 로드 실패: {e}")
                self.model = None
        else:
            logger.info("사전 학습된 XGBoost 모델을 찾을 수 없거나 xgboost 패키지가 설치되지 않았습니다. 현재는 규칙 기반(Rule-based) Re-ranking을 사용합니다.")

    def _extract_features(self, query: str, chunk_text: str, vector_score: float) -> np.ndarray:
        """
        ...
        numeric_pattern = r'\d+[\s]*(m|km|제곱미터|㎡|대|%|만원|미터|제곱|이상|이하|미만|초과)'
    ...
    def rerank_chunks(self, query: str, docs_with_scores: List[Tuple[str, float]], top_k: int = 3) -> List[str]:
        """
