...
DSN = os.environ.get(
    "PGIS_DSN", "postgresql://postgres:omnisite@127.0.0.1:55432/omnisite"
)
WORK_SRID = 5186
R_COVER = 300.0
MIC_TOL = 0.05
...
EXCL_RADIUS = (
    30.0  # 배제 버퍼 대표값 — 레이어별 반경은 도메인 값이라 여기선 고정 대표치
)
...
def _row(label: str, gp: float, push: float, query: float, fetch: float) -> None:
    ...
    print(
        f"  {'':<22} postgis   push {push:6.2f}  query {query:6.2f}  fetch {fetch:6.2f}"
    )
    print(
        f"  {'':<22}           DB상주 {resident:6.2f}s {_x(resident)}   "
...
def main() -> int:
    ...
    with psycopg.connect(DSN, autocommit=True) as conn, conn.cursor() as cur:
        ...
        print(
            f"  PostGIS {pv.split()[0]}  GEOS {pgeos.split('-')[0]}   "
        ...
        with T("gp_pip"):
            pts = S.points_in_parcels(
                parcels, spacing=SPACING, max_per_parcel=400, verbose=False
            )
        print(f"  후보점 {len(pts):,}")
        ...
        print(
            f"  {'':<22} ※ fetch 는 {len(got):,}행 × 3열을 파이썬으로 옮기는 비용이다"
        )
        ...
        for wm in ("4MB", "256MB", "1GB"):
            ...
            print(
                f"    work_mem {wm:>6}   query {T.get(f'tune_q_{wm}'):5.2f}  "
        ...
        print(
            f"    PostGIS 집계형 {T.get('agg'):5.2f}s (반환 {len(agg):,}행)  vs  "
