...
from app.api.v1 import auth, lands, ahp, simulations, audit, stakeholders, report
try:
    from app.api.v1 import pipeline, upload
except ImportError:
    pipeline = None
    upload = None
...
app.include_router(lands.router, prefix=settings.API_V1_STR + "/lands", tags=["Lands & HITL"])
...
app.include_router(report.router, prefix=settings.API_V1_STR + "/report", tags=["Report Generation"])

if upload and hasattr(upload, "router"):
    app.include_router(upload.router, prefix=settings.API_V1_STR + "/upload", tags=["Regulation & File Upload"])
if pipeline and hasattr(pipeline, "router"):
    app.include_router(pipeline.router, prefix=settings.API_V1_STR + "/pipeline", tags=["Pipeline Run"])

@app.get("/")
def read_root():
    return {"message": "OmniSite AI API Services Active"}
