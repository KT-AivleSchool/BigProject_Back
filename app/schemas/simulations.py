...
class SimulationRunRequest(BaseModel):
    ...
    ahp_model_id: Optional[int] = Field(None, description="적용할 가중치 락 세트 ID (생략 시 기본 락 세트 적용)")

class ScenarioDetail(BaseModel):
    scenario_type: str = Field(..., description="시나리오 종류 (A: 주민 우세, B: 상인 우세, C: 공공 조정)")
...
class SimulationResultResponse(BaseModel):
    ...
    conflict_sensitivity_score: float = Field(..., description="갈등 민감도 지수 (CSS - 0.0 ~ 10.0)")
    conflict_factors: Dict[str, float] = Field(
        ...,
        description="상세 갈등 인자 영향도 (예: 소음피해, 임대료상승, 보행혼잡, 경관훼손 등)"
    )
    scenarios: List[ScenarioDetail] = Field(..., description="3대 시나리오 예측 모델 전개 정보")
