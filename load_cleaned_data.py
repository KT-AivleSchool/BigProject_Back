...
# ─────────────────────────────────────────────────────────────────
POINT_SPEC = [
    ("01_버스정류소_유동인구_v2.csv", "bus_stop_passenger_stats",
     [("정류소명", "stop_name"), ("월평균승객수", "monthly_avg_passengers")]),

    ("02. 용산구_가로휴지통.csv", "street_trash_bins",
     [("설치주소", "installation_address")]),

    ("06. 용산구_공원데이터.xlsx", "parks",
     [("시설이름", "facility_name")]),

    ("07_담배꽁초_상습_무단투기_v3.csv", "cigarette_litter_hotspots",
     [("지번주소", "parcel_address")]),

    ("09. 서울특별시_용산구_흡연구역.csv", "smoking_areas",
     [("서울특별시 용산구 설치 위치", "installation_location")]),

    ("10. 소상공인시장진흥공단_상가.csv", "commercial_shops",
     [("도로명주소", "road_address"), ("상권업종대분류명", "business_category")]),

    ("용산구_CCTV.csv", "cctv_locations",
     [("구분", "location_description")]),

    ("용산구_공공와이파이.csv", "public_wifi_locations",
     [("구분", "location_description")]),

    ("용산구_공중화장실.csv", "public_toilets",
     [("구분", "location_description")]),

    ("용산구_소방용수시설_v2.csv", "fire_water_facilities",
     [("소재지도로명주소", "road_address")]),

    ("용산구_문화행사.csv", "cultural_event_locations",
     [("장소명", "place_name")]),

    ("용산구_공영주차장.csv", "public_parking_lots",
     [("주차장명", "parking_lot_name"), ("소재지도로명주소", "road_address"),
      ("소재지지번주소", "parcel_address")]),

    # ★ 신규 17번
    ("국유부동산_위경도_v2.csv", "national_properties",
     [("소재지(지번)", "parcel_address"), ("지목(공부)", "land_category"),
...
for fname, table, mapping in POINT_SPEC:
    ...
    for _, r in df.iterrows():
        ...
        rows.append(tuple(vals) + (float(r["경도"]), float(r["위도"]),
                                   float(r["경도"]), float(r["위도"])))
...
rows = [(r["시설종류"], r["기준"], r["게이트_WKT"], r["게이트_WKT"])
...
rows = [(str(r["역명"]).strip(),
        ...
        for _, r in s.iterrows() if pd.notna(r["역명"])]
execute_values(
    cur,
    "INSERT INTO subway_station_passenger_stats "
    "(station_name, total_passengers) VALUES %s", rows)
...
cur.execute("UPDATE candidate_lands SET area_m2 = ST_Area(geom_5186) "
...
print("\n=== 검증 ===")
TABLES = [
    "bus_stop_passenger_stats", "street_trash_bins",
    "subway_station_passenger_stats", "living_population_stats",
    "candidate_lands", "parks", "cigarette_litter_hotspots",
    "smoking_area_polygons", "smoking_areas", "commercial_shops",
    "cctv_locations", "public_wifi_locations", "public_toilets",
    "fire_water_facilities", "cultural_event_locations",
    "public_parking_lots", "national_properties",
]
for t in TABLES:
