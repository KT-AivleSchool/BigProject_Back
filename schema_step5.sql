...
--
-- 왜 필요한가
--   `hearing_result_a` 는 ORM · `schema.sql` · 실제 DB 가 **셋 다 달랐다.**
...
-- ── 1. hearing_result_a — 우리 산출물이 갈 컬럼 3개 추가 ─────────────────
--
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'hearing_result_a_parcel_id_fkey'
    ) THEN
        ALTER TABLE hearing_result_a
            ADD CONSTRAINT hearing_result_a_parcel_id_fkey
            FOREIGN KEY (parcel_id) REFERENCES booth_candidates(id) ON DELETE CASCADE;
    END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_hearing_result_a_parcel
    ON hearing_result_a(parcel_id);
...
ALTER TABLE hearing_result_a ALTER COLUMN parcel_id SET NOT NULL;

COMMENT ON COLUMN hearing_result_a.parcel_id IS
    '토론 대상 후보점. booth_candidates.id (런타임 값). candidate_land_id 와 id 공간이 다르다. '
    'NOT NULL — 이 테이블엔 run_id 컬럼이 없어 run 에 닿는 경로가 이 조인 하나뿐이다';
COMMENT ON COLUMN hearing_result_a.candidate_land_id IS
    '위 후보점이 놓인 필지. booth_candidates.land_id 에서 유도해 채운다';
COMMENT ON COLUMN hearing_result_a.result_json IS
    'STEP5 최종 산출물 전체(원본). 아래 개별 컬럼은 여기서 뽑아낸 사본이다';
COMMENT ON COLUMN hearing_result_a.normal_scenario IS
    '시나리오 B(조건부 타결). 엔진은 매 실행마다 A/B/C 중 1개만 낸다 — 나머지 둘은 NULL 이 정상이다';
COMMENT ON COLUMN hearing_result_a.optimal_scenario IS
    '시나리오 A(원만한 타결). 도출되지 않았으면 NULL';
COMMENT ON COLUMN hearing_result_a.worst_scenario IS
    '시나리오 C(협상 결렬). 도출되지 않았으면 NULL';
COMMENT ON COLUMN hearing_result_a.confidence_score IS
...
CREATE TABLE IF NOT EXISTS debate_logs (
    ...
    simulation_id INTEGER NOT NULL
                  REFERENCES hearing_result_a(id) ON DELETE CASCADE,
...
COMMENT ON TABLE  debate_logs IS
    'STEP5 공청회 토론 발화 1건 = 1행. hearing_result_a.result_json 의 debate_logs 와 같은 내용이다';
