...
`_new_run_id`(`pipeline_runner.py:667`)는 `runs/r_<날짜>_*` **폴더를 세어** 다음 번호를
매긴다. 폴더를 지우면 번호가 되돌아간다. 이 절의 `r_20260810_001~004`(도메인
`흡연업로드`)는 폴더 정리 후 **같은 이름의 다른 run** 으로 덮였다(2026-08-10 실측):
