...
| DELETE | `/api/v1/pipeline/runs/{run_id}` | **실행 취소** (3-4) |
...
- ✅ **마이페이지 API 를 2026-08-12 에 만들었다** — `GET /pipeline/runs?mine=true`
  (아래 3-3-1). 그전까지 있던 건 「적는 쪽」뿐이었다. ⚠ 「**담당은 천명님**」이라고
  적어뒀던 건 지난다(2026-08-12 프런트 세션 요청 · 사람 지시로 우리가 구현).
  `run_records` 자체도 같은 경위로 우리가 만들었다(4계층 문서 §6·§7).
- 🔴 **프런트에는 그 화면이 이미 있었고, 이미 부르고 있었다**(2026-08-12 실측).
  ...
  **`GET /api/v1/pipeline/runs?mine=true`**.
  ...
  ✅ 프런트는 삼키던 `catch` 를 없앴다(2026-08-12) — 그 덕에 화면에
  「HTTP 405 — Method Not Allowed」가 그대로 떴고 그래서 이 칸이 비어 있다는 게 보였다.
  **실패 분기를 빈 목록 분기보다 앞에** 뒀다: 순서가 반대면 「아직 실행한 분석 내역이
  없습니다」가 떠서 사용자가 자기 기록이 지워진 줄 안다 — 실패했을 때의 `[]` 는
  **「없다」가 아니라 「모른다」**다.
  ✅ **405 는 2026-08-12 에 해소됐다** — 같은 경로에 `GET` 을 더했다(아래 3-3-1).
  프런트가 이미 굳혀둔 응답 모양 —
  ...
  · 🔴 `mine=true` 인데 익명 행까지 돌려주는 셈이라 **이름과 내용이 어긋난다.**
    ...
    🔴 `is_mine: true` 만 돌려주면 익명 행이 **영원히 안 보이는데 에러가 안 난다** —
    로그인 전에 돌린 run 을 볼 방법이 사라진다.
    만드는 사람은 **이름(`mine`)이 아니라 이 문장**을 따를 것.
    ⚠ 여기 적힌 「구획」이라는 낱말은 **그때의 화면 계획**이다 — 실제 화면은 익명을
    감추고 **감춘 수를 적는** 쪽으로 만들어졌다. 서버 규약은 안 바뀌었다(§3-3-1 참조).
  · 🔴 **`GET /runs` 를 붙이는 순간 `POST /runs` 와 경로가 같아진다** — 인증 규약이
    메서드별로 갈린다(POST 선택 · GET 은 필수여야 「내 것」이 뜻을 갖는다).
- ✅ **화면 문구 「주인 미상(이관 전)」은 프런트가 고쳤다**(2026-08-12 회신 —
  「로그인 없이 실행된 분석 내역」). 「이관 전」이 안 되는 이유(익명은 미구현이 아니라
  의도된 정상 상태이고 소급 귀속 경로가 **없다**)는 1절 · 4계층 문서 ㉠ 에 있다 —
  여기 적어두면 정본이 둘이 된다.

### 3-3-1. `GET /pipeline/runs?mine=true` — 마이페이지 run 이력 (2026-08-12 신설)

```
GET /api/v1/pipeline/runs?mine=true[&limit=100]
...
- 🔴 **`mine=true` 인데 「내 것 + 익명」을 준다.** 이름과 내용이 어긋나 보이지만 위
  문단의 결론이다 — 프런트가 한 번 불러 `is_mine` 으로 가른다.
  구현은 `WHERE user_id = :me OR user_id IS NULL` 이다. `WHERE user_id = :me` 로
  거르면 익명 행이 **영원히 안 보이는데 에러가 안 난다**(원칙 4).
  ⚠ ~~「프런트가 **두 구획**으로 가른다」~~ — **화면은 그렇게 안 만들어졌다**
  (프런트 작업일지 `2026-08-12d.md` §2.1, 사람 지시 「자기 기록만」). 지금 프런트는
  익명 행을 **감추고 감춘 수를 적는다**(「로그인 없이 실행된 19건은 표시하지
  않습니다. 지워진 것이 아니라 **주인이 없는** 기록입니다」).
  🔴 **그래도 서버는 그대로 준다** — 이유가 없어진 게 아니라 **바뀌었다**:
  「19건을 감췄다」고 말하려면 그 19건이 **응답에 있어야** 센다. 서버가 걸러 주면
  화면은 감춘 사실조차 모르고 **20건이 1건으로 조용히 줄어든다.**
  구획이 몇 개인지는 **화면의 결정**이고, 계약이 붙드는 것은 **무엇을 주느냐**다.
- 🔴 **남의 행은 안 준다.** 「내 것 + 익명」이지 「전부」가 아니다 — 프런트가 가르는
  기준은 `is_mine` 뿐이라 남의 run 은 **익명과 구분이 안 된다**. 위 화면 문구대로면
  남의 run 이 「주인이 없는 기록」으로 세어진다 — 화면이 사실이 아닌 말을 한다.
- **인증 필수**(`POST /runs` 는 선택). 토큰이 없거나 죽었으면 **401** — 익명으로
  떨어뜨리면 모든 행이 `is_mine: false` 가 되어 **로그인했는데 내 기록이 없는 화면**이
  된다.
- **`mine` 은 `true` 하나만 정의돼 있다.** 다른 값·누락은 **400**(원칙 1). 조용히 같은
  응답을 주면 나중에 「거르는 줄 알았다」가 된다.
- `status` 는 `last_known_status` **그대로**다(위 문단 — 셋뿐인 것 자체가 정보다).
- 정렬은 **`started_at` 내림차순**. 프런트는 정렬하지 않는다.
...
- 🔴 **상한이 있고, 자른 사실을 응답에 적는다**(우리가 정한 갈림길). 기본 100건 ·
  ...
  준다 — **안 적으면 사용자는 옛 run 이 지워진 줄 안다**(원칙 4). 프런트는 `truncated`
  가 참이면 「최근 N건만 보입니다(전체 M건)」를 화면에 적어야 한다.
- ⚠ **나머지에 닿는 수단은 화면에 없다**(2026-08-12 프런트 결정 · 미룸). 「더 보기」·
  페이지네이션은 안 넣었다 — 서버는 `?limit=` 을 **이미 500까지 연다**(백엔드 변경 0).
  🔴 안내문은 **사실 진술**이어야 하고 **약속이면 안 된다**: 「최근 100건만 보입니다
  (전체 M건)」는 참이지만 「더 보려면…」은 없는 수단을 약속한다(원칙 4 — 방금 지운
  「이관 전」과 같은 모양이다).
  🔴 방아쇠는 감각이 아니라 **값**이다 — `truncated` 가 실제로 `true` 로 오는 시점.
  ⚠ 「100건 넘는 사용자가 생기면」이라는 전제는 **실측과 다르다**: 익명 행은 **모든
  사용자의 목록에 들어가므로** 상한 100 은 사람별이 아니라 **공용**이고, `run_pruner`
  는 `runs/` 의 `.gpkg`·`.parquet` 만 지우지 **행은 안 지운다**(단조 증가).
  2026-08-12 실측 20행 중 **19행이 익명**이고 하루 18건이 찍힌 날이 이미 있다.
  ⚠ 기본값 100 을 올려서 미루지 않는다 — 그 상한은 「화면이 죽지 않게」 둔 것이라
  올리면 문제를 옮기면서 방어만 얇아진다.
- 구현 `app/api/v1/pipeline.py` · 대조
  `python app\tools\check_mypage_runs.py`(**48항목**, 실 DB·실 Redis, LLM 0회).
  🔴 대조기가 묻는 것 중 제일 중요한 건 **「익명 행이 응답에 들어 있는가」**다 —
...
### 3-4. `DELETE /pipeline/runs/{run_id}` — 실행 취소 (2026-08-12 신설)

돌고 있는 run 을 멈춘다. **자식 프로세스를 실제로 죽이고 나서** 응답한다.

| 응답 | 언제 |
|---|---|
| **204** (본문 없음) | 취소됨. 자식은 종료됐고 `status.json` 은 `failed` 로 닫혔다 |
| **404** | 없는 `run_id` |
| **409** | 이미 끝난 run(`succeeded`·`failed`) — 취소할 게 없다 |
| **409** | 자식을 죽였는데 실행 스레드가 20초 안에 안 끝났다 (아래) |

- 취소된 run 의 최종 상태는 **`failed`** 이고 `error` 는
  `"사용자가 실행을 취소했습니다."` 다. 🔴 **`cancelled` 라는 상태값을 새로 만들지
  않는다** — 프런트 폴링 종료 조건(4절)이 갈리고, 옛 프런트는 모르는 값을 만나
  영원히 폴링한다. 사유는 `status` 가 아니라 `error` 가 들고 있다.
- 중간에 끊긴 칸은 `steps[].status = "failed"` 다. **`done` 으로 적지 않는다** —
  done 이면 그 칸 산출물이 다 나온 것처럼 읽힌다(원칙 4).
- `awaiting_hitl` 도 취소할 수 있다. 게이트 대기는 "끝난 것"이 아니라 도메인을
  점유한 상태이고(1절 409), 취소가 없으면 **그 도메인은 서버 재시작 전까지 잠긴다.**
- 🔴 **인증이 없다** — `GET /runs/{id}`·`POST .../hitl/{gate}` 와 같다. 여기만
  인증을 붙이면 **익명 run 은 영원히 취소할 수 없다.** 익명은 미구현이 아니라
  의도된 정상 상태이고(3-3 ㉠) 나중에 주인을 채우는 경로도 없다 — 즉 「나중에
  로그인해서 지운다」가 성립하지 않는다.

🔴 **왜 「자식을 죽인다」가 계약에 들어가나.** `status.json` 만 고쳐 쓰는 취소는
취소가 아니다. 두 가지가 동시에 어긋난다 —

1. 자식(파이프라인 CLI)은 계속 돌아 **정본 캐시·산출물 디렉터리를 마저 갈아엎는다.**
   화면은 「멈췄다」고 하는데 디스크는 계속 바뀐다(원칙 4).
2. 실행 스레드는 자기 메모리의 `doc` 를 들고 있다가 다음 단계 전이에서
   `status.json` 을 **통째로 덮어쓴다** → 취소 표시가 사라지고 run 은 `succeeded`
   로 끝난다. 밖에서 쓴 상태는 살아남지 못한다.

그래서 순서가 정해져 있다: **취소 접수 → 자식 종료(`terminate` → 5초 → `kill`)
→ 실행 스레드가 자기 `finally` 에서 단 한 번 뒷정리** (도메인 잠금 반납 ·
`finished_at` · `run_records` 갱신 · `status.json` 쓰기). 밖에서 대신 정리하지
않는다 — 두 곳이 같은 자리를 쓰면 어느 쪽이 이겼는지 알 수 없다.

⚠ 스레드가 20초 안에 안 끝나면 **409** 다. 이때 도메인 잠금은 **반납하지 않는다** —
살아 있는 스레드가 그 도메인을 아직 쓰고 있는데 잠금을 풀면 같은 도메인 run 이
둘 돌게 된다. 「취소했다」고 답하고 실제로는 안 끝난 것보다, 안 됐다고 말하는 게 낫다.

⚠ 취소를 접수한 뒤 응답 전에 run 이 **스스로 완주**했으면 **409** 다(좁은 틈).
완주한 run 을 취소로 적으면 그건 취소가 아니라 위조다.
...
- 값 범위 위반 · 알 수 없는 필드명 (**항목 안쪽 키도 본다** — §7-7-1)

⚠ 예전에 여기 있던 「`editable: false` 인 항목을 수정하려 함」은 **없앴다**(2026-08-12).
`editable` 이 항상 true 라 닿을 수 없는 분기였다 — 근거와 되살릴 조건은 §7-4.
...
```json
{"kind": "exclusion",   "dataset_id": "01", "role_index": 0,
 "editable": true, "confirmed": true,
...
{"kind": "intent",      "dataset_id": "12", "editable": true, "confirmed": false,
 "summary": "…", "message": "…", "current_roles": ["…"],
 "choices": [{"value": 1, "label": "가점(수요)", "needs_weight": true}, …]}

{"kind": "code_prefix", "dataset_id": "04", "op_index": 3,
 "editable": true, "confirmed": true,
...
🔴 **`editable` 과 `confirmed` 는 뜻이 다르다**(2026-08-12 분리 · PR #257 ⑴).

```
editable  = 지금 고칠 수 있는가    ·    confirmed = 이미 정해진 값인가
```

**`editable` 은 지금 항상 `true`** 다. 2026-08-05 에는 확정분을 `false` 로 냈고 근거는
「HITL 전 `confirmed` 는 조례·코드표에서 근거를 확실히 찾았을 때뿐이라 고칠 이유가
없다」였다. 그 근거가 무너졌다 — 조례 대조는 2026-08-10 에 **확정에서 제안으로
강등**됐고(§7-7), 자동 확정 경로 둘을 없앤 지금 남은 `confirmed` 는 **앞선 게이트
답변** 뿐이다. 자기가 방금 넣은 값을 못 고치면 화면은 「다시 시작」 말고 길이 없다.

🔴 그래서 `editable` 을 true 로 **바꾸기만 하지 않았다.** 그러면 「이미 확정된
항목」이라는 사실이 산출물에서 **사라진다** — 그 사실을 들고 있던 필드가 `editable`
하나뿐이었기 때문이다(원칙 4). 화면은 `confirmed: true` 를 **「확정됨 · 수정 가능」**
으로 표시한다. 확정분도 목록에 나온다 — 감추면 사람은 무엇이 이미 정해졌는지 모른 채
남은 것만 답하게 된다.

⚠ **`editable: false` 를 400 으로 막던 가드는 없앴다.** `editable` 이 항상 true 라
닿을 수 없는 분기이고, 그 `q` 는 **방금 우리가 만든 dict** 라 되읽으면 항등식이다.
없어진 규칙을 광고하는 코드는 다음 사람이 되살린다. 나중에 `editable: false` 를
되살리려면 **그때 가드도 같이** 만든다.

> 흡연 픽스처 실측: 질문 **6건**(배제 5 · 의도 0 · 지역코드 1). 배제 5건은
> `_prepare_dirs` 가 run 안 사본을 되돌리므로 `confirmed: false`, 지역코드 1건은
> `confirmed: true` 다. 즉 픽스처로 `hitl` 을 돌리면 게이트A 는 **확인 화면이 아니라
> 답해야 하는 화면**이다(§7-7). 빈 답 `{}` 로 통과하면 STEP2 가 `SystemExit` 이다.
...
- 승격은 roles 를 통째로 갈아치우므로 **옛 flag 의 확정 표시를 지운다.**
  ...
  ⚠ 2026-08-12 부로 굳는 건 `editable` 이 아니라 **`confirmed: true`** 다(§7-4).
  못 고치게 되진 않지만 **이미 정해진 값이라고 거짓말한다** — 지우는 이유는 그대로다.
...
**① 게이트 로직 단위 — 60/60 통과** (`app\tools\check_hitl_gate.py 흡연`, LLM 호출 0회)
(2026-08-12 `editable`↔`confirmed` 분리로 57 → 60. 뺀 항목은 없다)
...
- 게이트A 질문 — 기존 run 의 `reviewed.json` 에서 배제 **5**(2026-08-10 이전엔 3 —
  flag 없는 role 2건을 안 물었다) · 의도 0 · 지역코드 1.
  🔴 「전부 읽기 전용」은 **2026-08-12 에 뒤집혔다**(§7-4) — 지금 보는 것은
  `editable` 6/6 true 이고 `confirmed` 는 `04` 하나만 true 인가다. 옛 항목은
  **지우지 않고 뒤집었다**: 지우면 「예전엔 못 고쳤다」는 사실이 대조기에서 사라진다
  · `op_index` 로 실제 op(`filter_by_code_prefix`)를 찾는지
- 게이트A 답변 — 없는 대상 400 · 알 수 없는 필드 400 ·
...
| 요청 필드 | 뜻 | 기본 | 검증 |
|---|---|---|---|
| `user_input` | 사용자 의도. STEP0.5 가 여기서 **시설·지역을 확정**한다 | **없음(필수)** | 빈 문자열·공백만 → 400 · `--` 로 시작 → 400 · **400자** 초과 → 400 |
| `topn` | STEP4 가 뽑을 후보 개수 = 화면4 목록 길이 | **20** | 정수 아님 → 400 · 1~200 밖 → 400 |

🔴 `user_input` 상한은 **2026-08-24 에 200 → 400 으로 올렸다**(프런트 합의).
   화면1 이 「지역 + 시설 + 목적」을 한 칸에 조합해 보내면서 200 자가 실제로 모자랐다.
   서버 정본은 `pipeline_runner.USER_INPUT_MAX` **하나**이고 검사와 오류 문구가 같은
   상수를 본다 — 예전엔 둘 다 리터럴이라 한쪽만 고치면 「상한 200」이라 말하면서
   400 까지 받는 상태가 된다(원칙 4). ⚠ `topn` 옆칸의 `200` 은 **다른 값**이다
   (`TOPN_MAX`). 두 상수가 두 줄 떨어져 있으니 같이 고치지 말 것.
...
⚠ 「`04` 는 `editable:false` 라 건너뛰고」는 **그때의 사실**이다(고치지 않고 남긴다).
2026-08-12 부터 `editable` 은 항상 true 이고 그 자리는 `confirmed: true` 가 맡는다(§7-4).
지금 같은 실행을 하면 질문 6건의 값은 `editable` 6/6 true · `confirmed` 는 `04` 만
true 다(실측 2026-08-12).
