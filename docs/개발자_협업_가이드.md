...
> **🔴 정정 (2026-08-20).** 아래 경로는 **현행 표면이 아닙니다.** 업로드 API 는 2026-08-09 에
> 재작성됐고(`app/api/v1/upload.py`), 그때 저장 위치가 `uploads/regulations/` 에서
> `datasets/<도메인>/law/` 로 바뀌었습니다 — 옛 위치는 **파이프라인이 읽지 않는 곳**이라
> 올려도 아무 영향이 없었는데 `200` 이 나와서 안 걸렸습니다.
> 현행 경로는 서버를 띄우고 `http://127.0.0.1:8000/docs` 에서 확인하세요(그게 정본입니다).
...
## 🖨️ 4. PDF 렌더러 — ~~WeasyPrint~~ **playwright(chromium)**

> **🔴 정정 (2026-08-20).** 아래 원문은 **틀렸습니다.** PDF 보고서는 WeasyPrint 를 쓰지 않습니다.
> `app/services/pdf_service.py:4` 가 `from playwright.async_api import async_playwright` 이고
> `:19-20` 이 `p.chromium.launch(headless=True)` 로 렌더합니다. 저장소 전체에서 weasyprint
> **코드 참조는 0회**입니다.
>
> 이 문단을 지우지 않고 남기는 이유: 이 안내를 근거로 프런트 작업목록에 한동안
> **"weasyprint GTK 미설치로 화면6 막힘"** 이 요구사항으로 올라가 있었습니다. 지운 채로 두면
> 왜 그런 요구사항이 있었는지가 사라집니다.
>
> **지금 필요한 것**: Pango·Cairo 설치는 **필요 없습니다.** 대신 chromium 바이너리가 필요하고,
> `Dockerfile` 이 `mcr.microsoft.com/playwright/python:v1.48.0-noble` 를 베이스로 써서 이미
> 구워져 있습니다. 로컬에서 도커 없이 돌린다면 `playwright install chromium` 을 한 번 칩니다.
> ⚠ `constraints.txt` 의 `playwright==1.48.0` 핀을 풀지 마세요 — 패키지 버전이 올라가면
> 이미지에 구워진 브라우저 경로와 어긋나 **첫 PDF 요청에서만** 500 이 납니다(빌드·기동은 통과).

<details><summary>원문 (틀림 · 이력 보존용)</summary>
