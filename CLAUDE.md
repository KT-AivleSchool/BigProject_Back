...
| 🔴 **자식 하나가 호스트를 죽였는데 상한을 걸었더니 이번엔 사유가 낱말 하나가 됐다** | 2026-08-24 EC2 에서 `EV 충전소_2` STEP4 가 **커널 전역 OOM**(`constraint=CONSTRAINT_NONE … global_oom` · anon-rss **1,692MB** / VSZ 5,282MB · 호스트 총 **1,910MB**)으로 죽었다. 컨테이너 셋 다 `HostConfig.Memory = 0`(무제한)이라 **자식 하나가 호스트 전체를 끌고 갔다.** ⚠ 스왑 2G 는 **그때 이미 있었고**(2026-08-12 `/etc/fstab`) 막지 못했다 — 「램/스왑을 더 주면 된다」의 근거가 약하다. 실측(로컬 흡연): 피크 **905MB** 인데 그 자리는 MCLP 가 아니라 **`[D] 감쇠 계산`** 이고, 줄일 수 있는 쌍 배열은 **273.4MB(30.2%)** 뿐이라 **청킹만으로는 안 닫힌다.** 🔴 그리고 상한을 걸자 새 결함이 드러났다 — `MemoryError` 는 **메시지가 없어서** `_fail_reason` 의 트레이스백 갈래가 낱말 하나만 남긴다. **상한을 거는 목적이 바로 그 위치를 남기는 것**인데 거기서 잃으면 반쪽이다 | 축을 **커널이 보고한 것**에 맞춘다: `RLIMIT_AS`(VSZ)가 아니라 **`RLIMIT_DATA`**(brk + 익명 mmap ≈ numpy ≈ anon-rss)다. VSZ/anon-RSS 가 **3.12배**(pyogrio·GDAL 의 shp mmap + 예약)라 AS 로 걸면 **정상 실행이 죽는다**. 익명 mmap 이 DATA 에 잡히는 건 **리눅스 4.7+**(서버 `6.8.0-1062-aws` ✅). 🔴 값은 작업 예산이 아니라 **상자에서 나온 안전선**이다(`free -m` 1910 − 503 = **1400MB**, `OMNISITE_CHILD_MEM_MB`) — 이 상한은 자식을 **성공시키지 않는다.** 호스트를 안 죽이고 **사유를 남기며** 죽게 할 뿐이다. ⚠ 윈도우엔 `resource` 도 `preexec_fn` 도 없다 → `_mem_guard()` 가 **`None` 을 돌려준다**(no-op 함수를 주면 `ValueError`. `signal.SIGKILL` 과 같은 계열). ⚠ 설정은 **OS 분기보다 먼저** 읽는다 — 뒤에 두면 오타가 개발기에선 조용하고 **배포 서버에서만** 터진다. ⚠ `preexec_fn` 은 fork 와 exec 사이에서 도는데 러너는 멀티스레드다 → 본문은 **syscall 하나**(계산·포매팅·로깅 금지) |
