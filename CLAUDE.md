...
| 🔴 **여러 줄 예외의 마지막 줄만 남겨 화면이 애먼 파일을 지목했다** | 2026-08-24, 사용자 제보. 화면1 업로드 run 이 「실행이 실패했습니다. **출처: 행정동_크로스워크.csv**」로 죽었다 — 크로스워크는 **멀쩡했고** 같은 run 의 `[01] admin code=행정동코드 dongs=16` 이 그 파일로 정상 조인했다. 진짜 원인은 감리 AI 가 `[03] 관용 전기차 보유 현황`(**차량 명부** — 컬럼이 순번·차종·연식뿐)에 `좌표:stat_join` 을 붙인 것이고, STEP3 `_detect_admin_key_col` 은 그걸 **4줄로 정확히 말하고 있었다**: 「[03] … 찾지 못했습니다 / 컬럼: […] / 후보 매칭률: 후보 없음 / 참조: …csv」. 그런데 `_run_one` 이 자식 출력을 `line.strip()` 으로 담아 **들여쓰기를 지웠고**, `_fail_reason` 은 `[중단]` 마커가 없으면 `tail[-1]` 로 되돌아가 **꼬리표 한 줄만** `status.json.error` 에 남겼다. 예외가 안 나고 **지목만 틀린다** — `전기차_2`(08-23)·`EV 충전소_2`(08-24) **두 번 연속** 같은 오진을 유발했고, 사용자는 참조 데이터 쪽을 의심하며 물어왔다(원칙 4) | 사유를 고를 땐 **한 줄이 아니라 블록**으로 고른다. 파이썬 트레이스백은 「예외 줄은 들여쓰기가 없고 프레임 줄은 있다」로만 갈리므로 **strip 해서 담으면 시작을 못 찾는다** — 로그 tail 은 `rstrip()` 만 한다. 연쇄 예외면 **마지막** 트레이스백을 쓴다(그게 죽인 예외다). 🔴 **꼬리표는 대개 마지막 줄에 온다** — 「참조: …」·「이 명령을 치세요」류가 사유 자리를 차지하는 건 이번이 **두 번째**다(첫 번째는 `[중단]` 블록의 안내 명령어가 error 로 나간 건). 진단을 잘 적어도 **전달 경로가 한 줄이면 안 적은 것과 같다** |
| 🔴 **프롬프트 규칙이 「부재」만 보게 해서 검증 없는 주장이 산출물에 앉았다** | 위 줄의 **진짜 원인**(2026-08-24). 감리 프롬프트의 `coord_status` 규칙은 「좌표도 주소도 없는 통계 → `stat_join`」이라 **부재만** 본다. 차량 명부(순번·차종·연식)는 그 규칙을 **정확히 만족**한다 — **LLM 은 규칙을 지켰고 규칙이 틀렸다.** 그런데 `stat_join` 의 뜻은 「마스터·경계와 **조인해서** 위치를 붙인다」라 **조인 가능하다는 주장**이고, 규칙은 그 주장의 근거(조인 키)를 한 번도 안 본다. 아무도 검증하지 않으니 STEP3 `attach_layers` → `_detect_admin_key_col` 이 **처음 확인하는 자리**가 되고 거기서 파이프라인이 통째로 죽는다(`r_20260824_001` 10칸 중 **4칸째**). 🔴 **지뢰는 하나가 아니었다** — 같은 run 의 `[05] 전기차대수`(`시도명·자치구명·연도·전기차 대수`)도 같다: `자치구명` 은 「구」로 끝나 이름 키가 아니고 `연도`(4자리)·`대수`(3자리)는 `\d{5,10}` 을 못 넘는다. `[03]` 만 고쳤으면 실패가 **한 지표 뒤로 밀렸을 뿐**이다 | 확인 가능한 사실은 코드가 조달한다(원칙 3) — 「조인 가능한가」는 **데이터로 판정되는 것**이지 의미·의도가 아니다. 프롬프트를 조이는 것으로는 안 끝난다: 조여도 그 답은 여전히 **검증 안 된 주장**이다. ✅ 2026-08-24 — `_enrich_stat_join` 이 감리 시점에 **원본 값으로** 판정하고(`stat_join_check`) 못 찾으면 `positive/negative_factor` 를 `reference_only` 로 강등한 뒤 게이트A 에서 사람에게 묻는다. 앞선 `_enrich_code_prefix` · `c86e136`(시설 관련성)과 **같은 모양**이다. ⚠ 판정기는 **가져다 쓴다** (`gam2_weight_model._detect_admin_key_col`) — 사본을 두면 감리와 STEP3 가 다른 답을 내고 그때 죽는 쪽은 STEP3 다. ⚠ **`unknown` 을 「조인 못 한다」로 바꾸지 않는다** (원본 없음·미지원 확장자). 판정을 못 했으면 역할을 안 건드리고 STEP3 가 예전처럼 시끄럽게 죽게 둔다 — 조용한 강등이 더 나쁘다(원칙 1). 미지원 확장자 `ValueError` 와 「조인 키 없음」 `ValueError` 가 **같은 예외형**이라 try 를 하나로 묶으면 전자가 후자로 둔갑한다. ⚠ `hard_exclusion` 은 강등 대상이 아니다 — 강등하면 배제가 **조용히 풀린다**. ⚠ 사람이 다시 가점으로 되돌리면 STEP3 는 또 죽는다: 그건 사고가 아니라 **사유를 화면에서 읽고 내린 결정**이다(위 줄 덕분에 진단이 보인다) |
| 🔴 **검증 명령이 앱과 다른 인터프리터를 쟀고, 실패해도 `rc=0` 이라 빈 파일이 조용히 남았다** | 2026-08-25 배포 후 `pip freeze` **전체 diff** 를 뜨다 나왔다. 컨테이너의 `pip` 은 **`/usr/local/bin/pip`** 인데 앱은 **`/opt/venv/bin/python`** 으로 돈다 — `docker exec … pip freeze` 가 **4줄**을 줬다. 기준선(112줄)과 그대로 대면 「**108개가 사라졌다**」가 된다. 🔴 그래서 `python -m pip freeze` 로 바꾸면 더 나쁘다: venv 에 pip 이 없어 `No module named pip` 을 찍고 **종료 코드는 0** 이다 → 파일로 리다이렉트하면 **빈 파일이 성공한 얼굴로 남는다**(diff 는 「전부 사라졌다」). 곁가지로 **앱 코드는 `/app` 이 아니라 `/code` 에 있다** — `/app` 으로 grep 해서 「배포된 파일이 없다」를 한 번 봤다 | 무엇을 재는지 **경로로** 확인한다(`which pip` ↔ 앱이 쓰는 `python`). 목록은 앱 인터프리터로 `importlib.metadata.distributions()` 를 돌려 만든다 — 거기가 실제로 import 되는 자리다. 🔴 **`rc=0` 은 「출력이 있다」가 아니다**: 리다이렉트로 만든 산출물은 **줄 수를 세고 나서** 쓴다(기준선과 자릿수가 다르면 도구를 의심한다 — 원칙 5). 「전부 사라졌다」류 diff 는 **변화가 아니라 측정 실패**일 확률이 높다 |
| ⚠ **경로 인자가 다른 id 를 받는데 404 문구는 「데이터가 없다」고 말한다** | 같은 날 화면6 PDF 를 **실제로 눌러** 확인하다 나왔다(빌드 성공은 근거가 못 된다 — playwright 는 첫 `chromium.launch()` 에서만 터진다). `GET /api/v1/simulations/report/38` 에 `hearing_result_a.id` 를 넣었더니 **404 「해당 필지의 심의 시뮬레이션 이력이 존재하지 않아…」**. 그 경로가 받는 건 **`parcel_id`** 다(`{parcel_id}`). 문구는 **「그 필지에 이력이 없다」**인데 실제로는 **딴 걸 물은 것**이라, 그대로 읽으면 「토론 결과가 안 남았다」로 진단이 뒤집힌다. `38` 의 `parcel_id` 는 `909` 였고 그걸로 치니 **200 · application/pdf · 211,604 bytes · `%PDF-1.4`** | 경로 인자의 **이름**을 `openapi.json` 에서 확인하고 친다 — 둘 다 정수라 **틀려도 형식 오류가 안 난다**. 404 를 「없다」로 읽기 전에 **내가 넣은 id 가 그 축의 값인지**부터 본다(원칙 5). ⚠ `/simulation`(단수)·`/simulations`(복수) 두 prefix 가 같은 핸들러를 가리킨다 |
| 🔴 **상한을 걸었더니 정상 실행이 전부 죽었다 — 「예약」으로 「거주」를 막으려 했다** | 2026-08-24 EC2 에서 `EV 충전소_2` STEP4 가 **커널 전역 OOM**(`constraint=CONSTRAINT_NONE … global_oom` · anon-rss **1,692MB** / VSZ 5,282MB · 호스트 총 **1,910MB**)으로 죽었다. 컨테이너 셋 다 `HostConfig.Memory = 0`(무제한)이라 **자식 하나가 호스트 전체를 끌고 갔다.** ⚠ 스왑 2G 는 **그때 이미 있었고**(2026-08-12 `/etc/fstab`) 막지 못했다 — 「램/스왑을 더 주면 된다」의 근거가 약하다. 실측(로컬 흡연): 피크 **905MB** 인데 그 자리는 MCLP 가 아니라 **`[D] 감쇠 계산`** 이고, 줄일 수 있는 쌍 배열은 **273.4MB(30.2%)** 뿐이라 **청킹만으로는 안 닫힌다.** 🔴 **그래서 건 상한(`RLIMIT_DATA` 1400MiB · `6842362`)이 배포 서버의 실행을 전부 막았다**(2026-08-25 `r_20260825_001`). 사용자가 화면에서 먼저 봤다 — 「자식이 시그널 11(SIGSEGV)으로 종료됨」. `run.log` 를 보면 STEP2 정제 8개 중 01·05·08 이 **`std::bad_alloc`**(GEOS 가 C++ 에서 할당 실패) 이고 뒤이어 SIGSEGV 다. 재현은 `preexec_fn` **하나만** 켜고 끄는 것으로 됐다 — 켜면 `std::bad_alloc`, 끄면 rc=0 | 🔴 **`RLIMIT_DATA` 는 「예약」(`VmData`)을 세고 커널 OOM killer 는 「거주」(anon-RSS)를 본다 — 다른 축이다.** 같은 작업을 재보면 **3.10배** 갈린다: `datasets/region_data/BND_ADM_DONG_PG.shp`(135MB·3,559행)를 `shapely.from_wkb` 로 읽는 동안 **VmData 1,501MB / VmHWM 484MB**. 1400 이라는 숫자는 윈도우 **RSS** 905MB 실측에서 나왔는데 **예약 축에 갖다 붙였다** — 「한 형식으로 잰 벽을 다른 형식에 적용했다」와 같은 계열이고, 이번엔 축이 **예약↔거주**였다. 🔴 **값을 조정해서 살릴 수 없다**: 정상 실행이 예약 1,501MB 를 요구하는데 호스트 물리 램이 **1,910MB** 다 — 정상 실행을 통과시키는 상한은 이미 램보다 크고, 그러면 아무것도 안 막는다. ⚠ `RLIMIT_AS` 는 더 나쁘다(VSZ/anon-RSS **3.12배** → 5GB 를 줘야 한다). **거주량을 묶는 건 rlimit 이 아니라 cgroup 뿐이다**(`RLIMIT_RSS` 는 현대 리눅스에서 no-op) → 제자리는 docker **`mem_limit`** 이고 크기는 예약이 아니라 **VmHWM** 으로 잡는다. 🔴 **틀린 축의 손잡이를 남겨두지 않는다** — 상수·`OMNISITE_CHILD_MEM_MB` 를 같이 지웠다(2026-08-26). 남기면 다음 사람이 「끄면 되는 안전장치」로 읽고 다시 켠다. ⚠ **없앤 것은 처치이지 문제가 아니다** — 「자식 하나가 호스트를 죽인다」는 여전히 열려 있다. 🔴 **개발기에서는 원리적으로 못 잡는다**: 윈도우엔 `resource` 도 `preexec_fn` 도 없어 이 갈래가 **아예 안 돈다**(`signal.SIGKILL` 과 같은 계열 — 그래서 `_mem_guard()` 는 no-op 함수가 아니라 `None` 을 돌려줘야 했다). **배포 뒤에 실제로 눌러 보는 것 말고 잡을 길이 없었다.** ⚠ 곁가지로 얻은 참인 것 둘은 남는다 — `_fail_reason` 의 「메시지 없는 예외 → `발생 위치:` 프레임 줄」 갈래(상한과 무관하게 유효하다) · `preexec_fn` 본문은 fork·exec 사이라 **syscall 하나**여야 한다는 것 |
...
python app\tools\measure_step4_memory.py <도메인>     :: STEP4 피크 RSS 를 **자식 밖에서** 잰다
                                                     :: ⚠ STEP4 를 **실제로 돌린다**(`datasets/step4_output/`
                                                     ::    를 덮어쓰고 끝나면 되돌린다. `--no-restore` 로 끔)
                                                     :: 🔴 in-process 로 재면 하네스 import 가 섞이고 무엇보다
                                                     ::    **배포되는 실행 형태가 아니게 된다** — 실제 파이프라인은
                                                     ::    자식 프로세스다(`_proc_of` stage "4")
                                                     :: 🔴 재는 것은 **RSS 뿐**이다. 윈도우 RSS(working set)는
                                                     ::    리눅스 anon-RSS 와 **같은 값이 아니고**, `psutil.vms` 는
                                                     ::    리눅스 VSZ 와 **다른 축**이다(pagefile commit) — 여기서
                                                     ::    읽을 것은 절대값이 아니라 **비율**이다
                                                     :: 실측(2026-08-26 흡연) 피크 **905.0MB** · 쌍 배열은 그중
                                                     :: **273.4MB(30.2%)** · 피크 자리는 MCLP 가 아니라 **[D] 감쇠**
...
D:\obsidian_claude\10_OmniSite\
  ...
  02_작업일지\2026-08-26.md           ← 🔴 **최신.** 전날 넣은 자식 메모리 상한(`RLIMIT_DATA`
                                       1400MiB)이 **배포 서버의 모든 실행을 죽였다** — 되돌리고
                                       (`67d9480`) 실제로 돌려서 닫았다(`r_20260825_002` 8칸
                                       succeeded · loaded 13/20). §2 가 핵심이다: **예약**
                                       (`VmData`)으로 **거주**(anon-RSS)를 막으려 했고 실측
                                       **3.10배**(1,501 ↔ 484MB)라 값을 조정해서 살릴 수 없다
                                       (정상 실행이 램보다 큰 예약을 쓴다). §3 은 같은 상한이
                                       **두 층**에서 터진 것 — GEOS 는 `std::bad_alloc`,
                                       파이썬은 `MemoryError` 인데 **`str()` 이 비어** 세 데이터셋이
                                       사유 없이 죽었다(`2def13a` 로 예외형을 같이 적는다).
                                       🔴 §5 — **없앤 것은 처치이지 문제가 아니다**(호스트 OOM 은
                                       그대로 열려 있고, 다음 처치는 cgroup `mem_limit` · 치수는
                                       **VmHWM**). 그리고 이 결함은 **개발기에서 원리적으로
                                       못 잡는다**(윈도우엔 `resource`·`preexec_fn` 이 없다)
  02_작업일지\2026-08-25.md           ← `user_input` 400자 상한 · `seeds/` 를
                                       도메인 하위폴더로 분리(흡연 4 · 전기차 1 · **_미분류 1**) ·
                                       실패 사유가 애먼 것을 지목하던 갈래 처치 ·
                                       **§4 푸시·배포와 배포 후 확인**(`back_deploy`
                                       `f792ab8..6929070` · Actions 10m12s).
                                       🔴 §4-2 가 핵심이다 — **로컬에서 못 재는 갈래를 배포
                                       서버에서 쟀다**(Windows 는 시그널 사망도 큰 양수라
                                       `rc < 0` 에 안 걸린다). 로컬 `SIG9` 자리가 리눅스에선
                                       **`SIGKILL`** 로 나온다. §4-3 `pip freeze` 전체 diff
                                       12개 이동 · **playwright·pymupdf 는 핀이 버텼다** ·
                                       §4-4 화면6 PDF 를 **실제로 눌렀다**(211,604 B).
                                       ⚠ §4-5 「감시가 시작됐다」는 **「만료가 일어났다」가
                                       아니다** — 지금 `awaiting_hitl` 이 0개라 닫을 대상이 없다
  02_작업일지\2026-08-12.md           ← `POST /pipeline/runs` **선택적 인증**((가)안)
                                       + 프런트 저장소 **첫 열람**(읽기 전용).
  ...
  04_이슈\2026-08-25_실패사유가_애먼_것을_지목한_세_갈래.md
                                     ← 🔴 **화면이 「실행이 실패했습니다. <사유>」로 엉뚱한 것을
                                       지목하면 여기부터.** 정본은 `_fail_reason(tail, rc)` 하나이고
                                       **사유를 고르는 순서가 곧 정책**이다(`rc<0` → `[중단]` →
                                       마지막 트레이스백 → 마지막 **비어 있지 않은** 줄 → 종료 코드).
                                       같은 계열 셋 — ① 여러 줄 예외의 마지막 줄만 남아 애먼 파일을
                                       지목(08-24) ② 시그널로 죽으면 자식이 **아무것도 못 찍어**
                                       정상 진행 로그가 사유 자리에 앉는다 ③ 폴백이 `tail[-1]` 이라
                                       **빈 줄**이 사유가 된다. 7갈래 실측표 + 배포 서버 재실측
