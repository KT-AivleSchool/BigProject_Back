    ...
    if os.path.exists(csv_file_path):
        try:
            with open(csv_file_path, mode="r", encoding="utf-8") as f:
                ...
                if headers:
                    rag_context_text += f"[참고 조례 데이터 항목: {', '.join(headers)}]\n"
                
                for row in reader:
                    rag_context_text += " - " + " / ".join(row) + "\n"
            print(f"✅ '{csv_file_path}' 파일을 성공적으로 불러와 AI 배경지식(RAG)에 주입했습니다.\n")
        except Exception as e:
            print(f"⚠️ '{csv_file_path}' 읽기 오류: {e}")
            rag_context_text = "조례 데이터 읽기 실패"
    else:
        print(f"⚠️ '{csv_file_path}' 파일이 없어서 기본 더미 데이터를 사용합니다.\n")
        rag_context_text = "테스트 조례: 주거지역 인근 10m 이내 금연구역 지정 (DB Vector Search 생략)"
            ...
            for node_name, node_state in output.items():
                ...
                if node_name == "reporter":
                    ...
                    if "final_scenarios" in node_state:
                        print(json.dumps(node_state["final_scenarios"], ensure_ascii=False, indent=2))
